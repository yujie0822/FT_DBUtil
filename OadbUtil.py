...
class Ora():
    ...
    def __getConn(db_info):
        if Ora.__pool is None:
            __pool = PooledDB(cx_Oracle,user = db_info['user'],password = db_info['pwd'],\
            dsn ="%s:%s/%s"%(db_info['host'],db_info['port'],db_info['db']),\
            mincached=20,maxcached=200)
        return __pool.connection()

#查询表的所有列
    def columns(self,table):
        sql = ["select lower(column_name)column_name from \
        user_tab_columns where table_name=upper('%(table)s')"]
...
#参数四：用于where条件，如 where 字段3=值3 and 字段4=值4，格式{'字段3':'值3','字段4':'值4'}
    def backup(self,table,userid,remark,cond_dict):
        ...
        if rows[0]["CNT"]==0:#注意oracle列名大写
            ...
            cond_stmt = ','.join([ '%s=:%s' %(k,k) for k in cond_dict.keys() ])
...
#执行sql，参数一：sql语句，参数二：参数字典{'字段1','值1','字段2':'值2'}
    def query(self, sql, args = {}):
        ...
        return self.get_rows(None,False)
